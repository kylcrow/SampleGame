...
-(id) init
{
	...
	if( (self=[super initWithColor:ccc4(255,255,255,255)] )) {
		...
		leftJoy.backgroundSprite = [[CCSprite spriteWithFile:@"joystickdpadback.png"] retain];
		leftJoy.thumbSprite = [[CCSprite spriteWithFile:@"joystickdpadfront.png"] retain];
		leftJoy.joystick = [[SneakyJoystick alloc] initWithRect:CGRectMake(0,0,128,128)];
		leftJoystick = [leftJoy.joystick retain];
		[self addChild:leftJoy z:1];
				
		SneakyJoystickSkinnedDPadExample *rightJoy = [[[SneakyJoystickSkinnedDPadExample alloc] init] autorelease];
		rightJoy.position = ccp(416,64);
		rightJoy.backgroundSprite = [[CCSprite spriteWithFile:@"joystickdpadback.png"] retain];
		rightJoy.thumbSprite = [[CCSprite spriteWithFile:@"joystickdpadfront.png"] retain];
		rightJoy.joystick = [[SneakyJoystick alloc] initWithRect:CGRectMake(0,0,128,128)];
		rightJoystick = [rightJoy.joystick retain];
		[self addChild:rightJoy z:1];
...
- (void)update:(ccTime)dt {
	...
	NSLog(@"direction left %i    direction right: %i", leftJoystick.direction, rightJoystick.direction);
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (rotateDiff < -180)
		rotateDiff += 360;    
    CGFloat rotateSpeed = 0.1 / 180; // Would take 0.5 seconds to rotate half a circle
